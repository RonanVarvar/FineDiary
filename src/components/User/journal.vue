           ...
           <subjectFullList :subjects="journalsList.subjects" :classNumber="journalsList.number" :selectedJournalId="this.$route.params.selectedJournalId"></subjectFullList>
            <div class="journaOpened">
                <div class="headerWithButtons">
                    <button class="buttonCalendar">Февраль, 5</button>
                    <div>
                ...
                <div class="table">
                    <div class="row tableHeader">
                        ...
                        <div class="tableCell makeRelative additionalPadding"> Оцінки <a href="#"><img class='placeRight top5px additionalPadding' src="./img/show_marks.png"/></a></div>
                    ...
                    <div class="row tableHeader alignLeft" v-for="(el,index) in journalsList.listOfPeopleWithMarks">
                        <div class="tableCell" v-for="(el2, index2) in el" v-bind:class="{ width50: index2 === 'mark', alignLeft:index2 === 'name'}"  v-if="index2!='id'">
                            <span v-show="index2 === 'name'">{{ index + 1 }}</span>
                            {{el2}}
                        </div>
                        <div class="tableCell homeTask NoAlign-itemsCenter">
                            <span>{{journalsList.homeTask}}</span>
                            <span class="makeRelative colorGreen"><a href="#"> Детальніше </a> <a href="#"><img class='placeRight' src="./img/download_sign.png"/></a> </span>
                        </div>
                        <div class="tableCell">
                            <a href="#"><img src="./img/red_circleInfo.png"/></a><span>{{journalsList.listOfPeopleWithMarks.length}}</span>
                            <a href="#"><img src="./img/info_sign.png"/></a><span>{{journalsList.listOfPeopleWithMarks.length}}</span>
...
<script>
    ...
    export default {
        ...
        created:function (){//requset to server
            ...
            this.journalsList = this.getDataFromServer();//вызываем функцию для получения данных с сервера. Передаем ей id класса и id предмета (на основании того что выбрал ЮЗЕР)
        },
        methods:{
            getDataFromServer(){
                //запрос к серверу сделать Передаем ей id класса и id предмета
                let data =
                    {number:"9-a", id:2, subjectSelected: { id:1, subject: "математика"}, homeTask: 'Вирішити приклад', info:['make task in 2 days'], listOfPeopleWithMarks:[
                    {name: 'Миша Грищев', mark: '10', id:1},{name: 'Катя Грищева', mark: '8', id:2},{name: 'Стас Кит', mark: '12', id:3},{name: 'Таня Кот ', mark: '6', id:4},
                    {name: 'Таня Мишина ', mark: '3', id:5},{name: 'Дима Форк ', mark: '10', id:6},{name: 'Саша  Мел', mark: '11', id:7},{name: 'Никита Кот ', mark: '6', id:8}
                    ], subjects:[{ id:1, subject: "математика"}, { id:2, subject: "физика"}, { id:3, subject: "физ-ра"}, { id:4, subject: "физика"}]};
...
<style>
    ...
    div#journal button.buttonCalendar{
        position: absolute;
        left:10px;
        color: black;
    }
